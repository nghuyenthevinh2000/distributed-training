...
func (node *Node) brpc(reqBody map[string]interface{}, callbackHandler func(req Request) error) {
	for _, id := range node.nodeIds {
		if id == node.nodeId {
			continue
		}

		node.rpc(id, reqBody, callbackHandler)
	}
}
...
var ErrorCodes = map[int]string{
	...
	34: "Obsolete map",
	35: "Precondition failed",
	36: "Term can't go backwards!",
	37: "Only candidate can become leader!",
	38: "Not a leader! Leader is empty!",
	39: "Log index out of range",
	40: "Wrong entry",
}
...
func (e RPCError) Error(value string) error {
	return fmt.Errorf("error %v (%d), with additional notes: %s", e.Text, e.Code, value)
}
